                  ...
                  return (
                    <div key={node.type}>
                      ...
                      <div
                        ...
                        className="flex items-center gap-3 px-3 py-2.5 rounded-lg hover:bg-white hover:shadow-sm cursor-grab active:cursor-grabbing transition-all duration-150 border border-transparent hover:border-gray-200"
                      >
                        <span className="text-lg">{node.icon}</span>
                        <span className="text-sm font-medium text-gray-700 flex-1">{node.label}</span>
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
                        ...
                        <div className="ml-4 mt-2 space-y-2 border-l-2 border-gray-200 pl-2">
                          ...
                          {mcpServers.noAuth.map((server) => (
                            <div
                              ...
                              className="flex items-start gap-2 px-2 py-2 rounded-md hover:bg-white hover:shadow-sm cursor-grab active:cursor-grabbing transition-all duration-150 border border-transparent hover:border-green-200 group"
                          ...
                          {mcpServers.requiresAuth.map((server) => (
                            <div
                              ...
                              className="flex items-start gap-2 px-2 py-2 rounded-md hover:bg-white hover:shadow-sm cursor-grab active:cursor-grabbing transition-all duration-150 border border-transparent hover:border-amber-200 group"
                              title={`Drag to add ${server.name} (requires ${server.apiKey})`}
                            >
                              <span className="text-base mt-0.5">{server.icon}</span>
                              <div className="flex-1 min-w-0">
                                <div className="text-xs font-medium text-gray-700 group-hover:text-amber-700">
                                  {server.name}
                                </div>
                                <div className="text-xs text-gray-500 truncate">
                                  {server.description}
